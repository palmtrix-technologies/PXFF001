...
{
    $this->db->where('Vehicle_number', $data['Vehicle_number']);
    $query = $this->db->get('n_v_vehicledetails');
    $count_row = $query->num_rows();
    if ($count_row > 0) {
       return "data already exist";
    } else {
       
        $this->db->insert('n_v_vehicledetails',$data);
...
public function selectcode()
{
  ...
  $this->db->select_max('vehicle_code');
$this->db->from('n_v_vehicledetails');
$query = $this->db->get();
$result = $query->result();
if($result==NULL)
...
public function update($id,$data)
{
    ...
    $this->db->where('vehicleid',$id);
        $this->db->update('n_v_vehicledetails',$data);
...
public function enable_status($id)
{
  ...
  if($this->db->update('n_v_vehicledetails'))
  {
    return 1;
  }else{
    return 0;
